...
get_latest_version() {
    ...
    API_RESPONSE=$(curl -sS "https://api.github.com/repos/portofolio-mager/ai-squad/releases")
    if [ $? -ne 0 ]; then
...
extract_and_install() {
    ...
    mv "${tmp_dir}/ai-squad${extension}" "$bin_dir/$INSTALL_NAME${extension}"
    rm -rf "$tmp_dir"
...
main() {
    ...
    RELEASE_URL="https://github.com/portofolio-mager/ai-squad/releases/download/v${VERSION}"
    ARCHIVE_NAME="ai-squad_${VERSION}_${PLATFORM}_${ARCHITECTURE}${ARCHIVE_EXT}"
